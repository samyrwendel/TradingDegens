"""A carteira REAL do analista, lida do site dele — SÓ PRO DONO (DA-148).
...
``ANALISTA_CARTEIRA_EMAIL`` no ambiente do serviço, e a URL da carteira no site
dele de ``ANALISTA_CARTEIRA_URL`` (mesma disciplina do
...
def _base() -> str:
    """URL da carteira no site do analista — dado pessoal como o e-mail: vem do
    ambiente (``ANALISTA_CARTEIRA_URL``), nunca do código."""
    return (os.environ.get("ANALISTA_CARTEIRA_URL") or "").strip().rstrip("/")
...
_CACHE = Path.home() / ".tradingagents" / "cache" / "analista-carteira.json"


def _email() -> str:
    return (os.environ.get("ANALISTA_CARTEIRA_EMAIL") or "").strip()


def configurado() -> bool:
    """A feature existe nesta instância? Sem credencial, ela não existe."""
    return bool(_email() and _base())
...
def _grava_cache(payload: dict[str, Any]) -> None:
    try:
        ...
        logger.info("cache da carteira do analista não gravado: %s", exc)
...
def _busca() -> dict[str, Any]:
    ...
    base = _base()
    r = s.post(f"{base}/login.php", data={"email": _email()}, timeout=_TIMEOUT_S, allow_redirects=False)
    if r.status_code not in (200, 302):
        raise RuntimeError(f"login recusado ({r.status_code})")
    r = s.get(f"{base}/api.php?acao=ler", timeout=_TIMEOUT_S)
    r.raise_for_status()
    dados = r.json()
    ...
    try:
        h = s.get(f"{base}/historico.json", timeout=_TIMEOUT_S)
        h.raise_for_status()
        historico = h.json()
...
def carteira(*, force: bool = False) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.info("leitura da carteira do analista falhou: %s", type(exc).__name__)
        if cache:
