"""Comparação de duas leituras + meta-juiz (Fase 3 do Modo analista).

Confronta DUAS análises do mesmo ativo — via o atalho "comparar" (roda Padrão +
analista de uma vez, task 017) ou o fluxo MANUAL (selecionar duas análises já
...
def method_label(method: str) -> str:
    return "Método do analista" if method == "analista" else "Padrão"


def detect_method(record: dict) -> str:
    """Infer a run's method from its stored result (analista if it wrote an analista
    ...
    ``analista_report`` vazio, e a inferência por ausência o classificava como
    ...
    analista de verdade. Inferir por ausência era o furo; agora o marcador manda.
    """
    ...
    if res.get("setup123"):
        return "setup123"
    return "analista" if (res.get("analista_report") or "").strip() else "padrao"
...
def confront_pair_valid(col_a: dict, col_b: dict) -> bool:
    """A confront is legitimate ONLY as Padrão × Analista on the SAME timeframe and
    ...
    if {col_a.get("method"), col_b.get("method")} != {"padrao", "analista"}:
        return False
...
def build_column(record: dict, method: str) -> dict:
    ...
    col = {
        ...
        "analista_report": r.get("analista_report", "") or "",
...
def meta_judge(a: dict, b: dict) -> dict:
    ...
    Works for any pair of the same ticker: Padrão × Analista (thesis vs timing) or two
    ...
    # é estado de ERRO. Os chamadores só passam Padrão × Analista, então isto é rede
    # de segurança — se disparar, algo montou o par errado.
    if a.get("method") == b.get("method") and a.get("method") in ("padrao", "analista"):
    ...
    if agree:
        ...
        divergencia = (
            ...
            + (f"\n- Timing (método do analista): {setup_pt}." if setup_pt else "")
        )
...
def _drop_nature_note(a: dict, b: dict) -> tuple[str, dict]:
    ...
    analista = (a if a.get("method") == "analista"
             else b if b.get("method") == "analista" else None)
    field = (analista or {}).get("drop_nature") or {}
...
def _same_method_error(a, b, la, lb, va, vb, da, db) -> dict:
    ...
    and points to the fix (run the missing method so it is a real Padrão × Analista)."""
    return {
        "agreement": "invalido",
        "verdict": None,
        "verdict_a": va, "verdict_b": vb,
        ...
        "label_a": la, "label_b": lb,
        "date_a": da, "date_b": db,
        "headline": "Confronto exige Padrão × Analista — não método contra ele mesmo.",
        "concordancia": "",
        "divergencia": (
        ...
        "significado": (
            ...
            "Padrão × Analista de verdade — a divergência entre os dois é o sinal."
        ),
    }
...
def _meaning(agree, have_both, va, vb, tfa, tfb, la, lb, diff_method, diff_tf, a, b) -> str:
    ...
    if diff_method:
        # Padrão (tese de fundo) × analista (timing pelo recuo à média)
        analista_is_a = a.get("method") == "analista"
        analista_v = va if analista_is_a else vb
        padrao_v = vb if analista_is_a else va
        analista_l = la if analista_is_a else lb
        if _vkey(cons) == _vkey(analista_v):
            return (
                f"O Padrão vê a tese de fundo (**{_vpt(padrao_v)}**), mas o método "
                f"do analista ({analista_l}) está mais cauteloso (**{_vpt(analista_v)}**): o timing "
                "ainda não confirmou. Leitura: esperar o recuo à média antes de agir "
                "com peso — entrar cedo é comprar contra o timing."
            )
        return (
            f"O método do analista ({analista_l}) vê um gatilho de timing (**{_vpt(analista_v)}**) "
...
# Back-compat alias: the auto "comparar" path (task 017) passes (padrao, analista).
