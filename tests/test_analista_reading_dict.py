"""O DICT do Método do analista (card da tela) == a DECISÃO do módulo (task 20260904-003).

`analista_reading_dict` compõe as MESMAS funções de `build_analista_method_section` (o
texto do analista `analista`). Este teste SOLDA os dois: a decisão do card (estado,
...
def test_o_card_carrega_LITERALMENTE_a_decisao_do_modulo(monkeypatch):
    ...
    d = analista_reading_dict("AVGO", "2026-09-03", "stock")
    md = build_analista_method_section("AVGO", "2026-09-03", "stock")
...
def test_baixa_sem_gate_e_caixa(monkeypatch):
    ...
    d = analista_reading_dict("GOOGL", "2026-09-03", "stock")
